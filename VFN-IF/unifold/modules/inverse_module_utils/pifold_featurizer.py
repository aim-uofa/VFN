...
class Pifold_featurizer(nn.Module):
    def __init__(self, cfg):
        ...
        # save mean and std
        # torch.save (mean,'dataset/xyz/mean.pt')
        # torch.save (std,'dataset/xyz/std.pt')
        # assert self.num_mask_edge>=0 and cfg.shuffle_aug and self.num_drop_edge==0
        # load mean and std
    ...
    def forward(self, S, X, mask, gt_frame,atoms14,atoms14_mask,atoms14_alt,atoms14_alt_mask,atoms14_ambiguous_mask):
        ...
        if V_xyz.max() > 3 or V_xyz.min() < -3: # TODO zmz: need to be modified
            #print("V_xyz", V_xyz.max(), V_xyz.min())
        ...
        # print("O", V_xyz[:10, 2, :])
        # else:
        #     output_path = 'dataset/xyz/'
        #     id = 0
        #     if not os.path.exists(output_path):
        #         os.makedirs(output_path)
        #     # determine the file name\
        #     while os.path.exists(output_path + str(id) + '.pth'):
        #         id += 1
        #     torch.save(V_xyz, output_path + str(id) + '.pth')
