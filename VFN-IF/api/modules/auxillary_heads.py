...
class InverseClsHead(nn.Module):
    def __init__(self, num_res_type, d_single, d_hid):
        super(InverseClsHead, self).__init__()

        self.num_res_type = num_res_type
        self.d_single = d_single
        self.d_hid = d_hid

        self.layer_norm = LayerNorm(self.d_single)

        self.linear_1 = Linear(self.d_single, self.d_hid, init="relu")
        self.linear_2 = Linear(self.d_hid, self.d_hid, init="relu")
        self.act = nn.GELU()
        self.linear_3 = Linear(self.d_hid, self.num_res_type, init="final")
