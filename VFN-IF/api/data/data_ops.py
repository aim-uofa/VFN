...
from api.config import N_RES, N_EXTRA_MSA, N_TPL, N_MSA
from api.data import residue_constants as rc
from api.modules.frame import Rotation, Frame
...
used_key = set(['aatype', 'residue_index','seq_length','all_atom_positions','all_atom_mask',
                'is_distillation','seq_mask','true_frame_tensor','frame_mask','chi_angles_sin_cos',
                'chi_mask','between_segment_residues','target_feat','between_segment_residues',
                'crop_and_fix_size_seed','resolution','num_recycling_iters','use_clamped_fape',
                'atom14_gt_exists','atom14_gt_positions','atom14_alt_gt_positions','atom14_alt_gt_exists',
...
def cast_to_64bit_ints(protein):
    ...
    for k, v in protein.items():
        if k.endswith("_mask"):
            ...
            protein[k] = v.type(torch.int32)

    return protein

#@cost_time
def cast_to_64bit_ints_fast(protein):
    # We keep all ints as int64
    # protein = pop_useless_key(protein)
    global used_key
    protein_new = {}
    for k in protein.keys():
        if k in used_key:
            if k.endswith("_mask"):
                protein_new[k] = protein[k].type(torch.float32)
            elif protein[k].dtype in (torch.int32, torch.uint8, torch.int8):
                protein_new[k] = protein[k].type(torch.int32)
            else:
                protein_new[k] = protein[k]
    # for k, v in protein.items():
    #     if k in used_key:
    #         if k.endswith("_mask"):
    #             protein_new[k] = v.type(torch.float32)
    #         elif v.dtype in (torch.int32, torch.uint8, torch.int8):
    #             protein_new[k] = v.type(torch.int32)
...
def correct_msa_restypes(protein):
    ...
    # protein["msa"][0,0] = 8000
    if (protein["msa"]>21).any():
        print('correct_msa_restypes')
        print('warning! detect msa feat larger than 21, auto convert to X(unknown)')
        protein["msa"][protein["msa"]>21]=20

    protein["msa"] = torch.gather(new_order, 0, protein["msa"]).long()

    return protein

def identity(protein):
    return protein
...
def nearest_neighbor_clusters(protein, gap_agreement_weight=0.0):
    ...
    if (protein["msa"]>23).any():
        print('summarize_clusters')
        print('warning! detect msa feat larger than 23, auto convert to X(unknown)')
        protein["msa"][protein["msa"]>23]=20
...
def make_masked_msa(
    ...
    if (protein["msa"]>22).any():
        print('summarize_clusters')
        print('warning! detect msa feat larger than 22, auto convert to X(unknown)')
        protein["msa"][protein["msa"]>22]=20
    categorical_probs = (
...
#@cost_time
@curry1
def make_masked_msa_skip(
    protein, config, replace_fraction, gumbel_sample=False, share_mask=False
):
    
    protein["bert_mask"] = (torch.rand_like(protein["msa"].float())>=0.5).to(torch.float32)
    protein["true_msa"] = protein["msa"]
    protein["msa"] = protein["msa"]
    ...
    for k, v in protein.items():
        ...
        for i, dim_size in enumerate(shape_schema[k]):
            if dim_size == N_RES:
                ...
                #     print('single_crop_time:{}; key:{}; v shape:{}; crop_idx shape:{}'.format(end_time - start_time, k, v_shape,crop_idx.shape))
        cropped_protein[k] = v
    return cropped_protein

#@cost_time
def pop_useless_key(features):
    global used_key
...
#@cost_time
def inverse_folding_featurizer(features,config):
    features = pop_useless_key(features)
    return features
