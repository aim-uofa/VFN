    ...
    if not common_cfg.fast_mode:
        ...
        operators.extend(
            [
                ...
                data_ops.correct_msa_restypes if not common_cfg.fast_mode else data_ops.identity,
        ...
        operators.append(data_ops.cast_to_64bit_ints_fast)
        operators.append(data_ops.squeeze_features)
        operators.append(data_ops.make_seq_mask)
        operators.append(data_ops.make_atom14_masks)
        operators.append(data_ops.make_target_feat)
...
def crop_and_fix_size_fns(common_cfg, mode_cfg, crop_and_fix_size_seed):
    ...
    if common_cfg.fast_mode:
        crop_feats = data_ops.pop_useless_key(crop_feats)
...
def ensembled_fns(common_cfg, mode_cfg):
    ...
    if mode_cfg.block_delete_msa and not multimer_mode and not common_cfg.fast_mode:
    ...
    if "masked_msa" in common_cfg and not common_cfg.fast_mode:
    ...
    elif "masked_msa" in common_cfg and common_cfg.fast_mode:
        operators.append(
            data_ops.make_masked_msa_skip(
                common_cfg.masked_msa,
                mode_cfg.masked_msa_replace_fraction,
                gumbel_sample=gumbel_sample,
                share_mask=mode_cfg.share_mask,
    ...
    if common_cfg.msa_cluster_features and not common_cfg.fast_mode:
        if v2_feature:
...
def process_features(tensors, common_cfg, mode_cfg):
    ...
    # if common_cfg.fast_mode:
    #     nonensembled.extend(label_transform_fn_fast())
    # elif mode_cfg.supervised and (not multimer_mode or is_distillation):
